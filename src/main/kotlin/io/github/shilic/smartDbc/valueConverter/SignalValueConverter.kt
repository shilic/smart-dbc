...
fun String.textToPhy(factor: Double, offset: Double, valueTable: Map<Int, String>) : Double = when {
    ...
    valueTable.count() == 0 -> runCatching { this.toDouble() }.getOrElse { e ->
        error("不存在值描述，并且输入的文本值 '$this' 无法转换为 Double : $e") }
    // 通过值描述直接获取原始的键, 并将总线值的键转换为物理值; 如果值描述查询不到，则抛出异常
    else -> valueTable.findFirstIndexByValue(this)?.toLong()?.hexToPhy(factor, offset)
        ?: runCatching { this.toDouble() }.getOrElse { e ->
            error("在值描述 '$valueTable' 中, 没有查询到值 '$this' 对应的键。" +
                    "尝试直接将文本值转换为物理值，并且输入的文本值 '$this' 无法转换为 Double : $e") }
}
/** 将16进制总线值转换为文本值 */
fun Int.hexToText(valueTable: Map<Int, String>): String = when {
    // 没有值描述, 直接返回值的字符串形式
    valueTable.count() == 0 -> this.toString()
    // 有值描述, 从值描述中取出值 (注意：索引必须是总线值); 如果没有值描述，则直接返回值的字符串形式
    else -> valueTable[this] ?: this.toString()
}
/** 将文本值转换为16进制总线值 */
fun String.textToHex(valueTable: Map<Int, String>): Int = when {
    // 没有值描述, 直接转换为 int 值
    valueTable.count() == 0 -> runCatching { this.toInt() }.getOrElse { e ->
        error("不存在值描述，并且输入的文本值 '$this' 无法转换为 Int : $e") }
    // 通过值描述直接获取原始的键, 如果值描述查询不到，则抛出异常
    else -> valueTable.findFirstIndexByValue(this)
        ?: runCatching { this.toInt() }.getOrElse { e ->
            error("在值描述 '$valueTable' 中, 没有查询到值 '$this' 对应的键。" +
                    "尝试直接将文本值转换为总线值，并且输入的文本值 '$this' 无法转换为 toInt : $e") }
}
