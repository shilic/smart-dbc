...
)
{
    ...
    private fun parseLines(reader: BufferedReader): DataBaseCanImp {
        ...
        reader.forEachLine { rawLine ->
            ...
            // 跳过匹配失败的行
            val lineStart = parseLineStart(line) ?: return@forEachLine
            try {
                when (lineStart) {
    ...
    companion object {
         ...
         * */
        operator fun invoke(file: File, encoding: Charset = file.encoding ?: DefaultCharset): DbcFileReader {
        ...
        fun parseLineStart(rawLine: String): String? {
            val line = rawLine.trim()
            return startRegex.find(line)?.groups["start"]?.value
        }
        /** 版本行正则表达式。*/
