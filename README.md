...
```kotlin
// 定义一个数据类，用于映射DBC中的报文；在类上使用 CanMessageBinding 注解绑定这个数据类属于哪一个DBC中的哪一个报文。
@CanMessageBinding(dbcTag1, msg1_Id)
data class Message1(
    // 使用 CanSignalBinding 注解，绑定这个字段(属性)，映射到DBC文件中的一个信号。
    @CanSignalBinding("msg1_sig1")
    var msg1sig1: Int = 0,

    @CanSignalBinding("msg1_sig2")
...
CanIo.apply {
    // 注册 DBC
    val dbc =  File("example.dbc")::inputStream.toDbc().apply {
        // 设置DBC标签, 这里需要和数据模型上用 CanMessageBinding 绑定的DBC标签一致。
        dbcTag = "myDbcTag"
    }
    // 将 DBC 添加到 DBC 管理器中(绑定DBC, 使其拥有CAN解析的能力)
    dbcMap[dbc.dbcTag] = dbc

    // 绑定数据模型 (需要在绑定DBC之后)（自动反射绑定所有 @CanSignalBinding 字段）
    binding(Message1())
    // 有多个数据模型则反复绑定
    binding(Message2())
...
```kotlin
// 更详细的用法请参考 src/test/kotlin/dbcDemo/DbcDemo.kt
...
// 又或者转换指定 sheet 为 DBC
val dbc: DataBaseCanImp =  ExampleExcelPath1.getDbc(
    sheetName = "CAN1",
...
│  │   MutableStateFlow<Msg1>                             │  │
│  │   MutableStateFlow<Cmd1>                             │  │
│  └───────┬──────────────────────────────┬───────────────┘  │
│          │ onListening()                │ transmit()       │
│          ▼                              ▼                  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │                    smart-dbc                         │  │
│  │   CanIo + @CanSignalBinding + DbcFileReader          │  │
│  └──────────────────────┬───────────────────────────────┘  │
│                         │                                  │
│                         ▼                                  │
│  ┌──────────────────────────────────────────────────────┐  │
...
@CanMessageBinding(dbcTag1, msg1_Id)
data class Msg1(
    @CanSignalBinding("status1")
    var status1: Int = 0,
    @CanSignalBinding("status2")
    var status2: Int = 0,
    // ... 省略其他 N 个开关量、传感器...
)

// 发送模型示例：指令
@CanMessageBinding(dbcTag1, msg2_Id)
data class Cmd1(
    @CanSignalBinding("Motor_start")
    var start: Int = 0,
    @CanSignalBinding("Motor_stop")
    var stop: Int = 0,
    // ... 省略其他代码
)
```

> **注意**：所有 `@CanSignalBinding` 注解的字段必须声明为 `var`。框架通过反射写入解码值，`val` 字段不会报错但永远接收不到数据。
...
这一步骤需要引入`CAN`卡厂家实现的`CAN`通信软件包来实现，他们一般在软件包里边实现了一些最基本的`CAN`收发操作，但是不会给你集成`CAN`报文的快速编解码；如果是在你的车载大屏上，一般是他们提供一个`AIDL`封装的组件；如果是在`Windows`上，则一般是一个`DLL`，例如周立功和同星的二次开发`DLL`，如需使用本框架，还需要你自己写一层`JNI`来适配（***或许是后续更新方向？***）。
...
- 优化：将软件包发布从 `Github Packages` 改为了`maven central`，现在可以更方便的添加依赖了。新增了更多`kotlin`风格的`API`
